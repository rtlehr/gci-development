...
const isEdit = false
const props = defineProps({ roles:{type:Array,default:()=>[]}, groups:{type:Array,default:()=>[]}, teams:{type:Array,default:()=>[]} })
const form = useForm({ person_code:'', first_name:'', alternate_first_name:'', preferred_name:'', last_name:'', alternate_last_name:'', company_name:'', email:'', employment_status:'', notes:'', group_ids:[], team_ids:[], role_ids:[], phone_numbers:[createEmptyPhoneNumber(true)], addresses:[createEmptyAddress(true)], attachments:[], existing_attachments:[], remove_attachment_ids:[] })
...
function submit(){ if(!validate())return; form.transform(data=>{const transformed={...data,attachment_meta:data.attachments.map((a,i)=>({category:a.category??'',description:a.description??'',is_primary:a.is_primary?1:0,sort_order:i})),new_attachments:data.attachments.map(a=>a.file).filter(Boolean)};delete transformed.attachments;delete transformed.existing_attachments;delete transformed.remove_attachment_ids;return transformed}).post('/people',{forceFormData:true}) }
