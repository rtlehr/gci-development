...
<div class="flex items-center justify-between"><div><h1 class="text-2xl font-semibold">Edit Person</h1><p class="mt-1 text-sm text-muted-foreground">Update person details, assignments, access, and files.</p></div><Button as-child variant="outline"><Link href="/people">Back to List</Link></Button></div>
...
<div class="space-y-2"><Label for="notes">Notes</Label><Textarea id="notes" v-model="form.notes" rows="5" /></div>
...
<div class="flex gap-3 border-t pt-5"><Button type="submit" :disabled="form.processing">{{ form.processing ? 'Saving...' : 'Save Changes' }}</Button><Button as-child variant="outline"><Link href="/people">Cancel</Link></Button></div>
</div></div></form></div>
</template>
<script setup>
...
import PersonSectionNav from '@/components/people/PersonSectionNav.vue'
import UserRoleEditor from '@/components/people/UserRoleEditor.vue'
...
const isEdit = true
const props = defineProps({ person:{type:Object,required:true}, selectedRoleIds:{type:Array,default:()=>[]}, roles:{type:Array,default:()=>[]}, groups:{type:Array,default:()=>[]}, teams:{type:Array,default:()=>[]} })
...
const form = useForm({ person_code:props.person.person_code??'', first_name:props.person.first_name??'', alternate_first_name:props.person.alternate_first_name??'', preferred_name:props.person.preferred_name??'', last_name:props.person.last_name??'', alternate_last_name:props.person.alternate_last_name??'', company_name:props.person.company_name??'', email:props.person.email??'', employment_status:props.person.employment_status??'', notes:props.person.notes??'', user_id:props.person.user_id??null, group_ids:(props.person.groups??[]).map(x=>x.id), team_ids:(props.person.teams??[]).map(x=>x.id), role_ids:(props.selectedRoleIds??[]).map(Number), phone_numbers:phones.length?phones:[createEmptyPhoneNumber(true)], addresses:addresses.length?addresses:[createEmptyAddress(true)], attachments:[], existing_attachments:existing, remove_attachment_ids:[] })
...
function submit(){ if(!validate())return; form.transform(data=>{const transformed={...data,_method:'put',attachment_meta:data.attachments.map((a,i)=>({category:a.category??'',description:a.description??'',is_primary:a.is_primary?1:0,sort_order:i})),new_attachments:data.attachments.map(a=>a.file).filter(Boolean),remove_attachment_ids:data.remove_attachment_ids??[]};delete transformed.attachments;delete transformed.existing_attachments;return transformed}).post(`/people/${props.person.id}`,{forceFormData:true}) }
