            ...
            <div class="min-w-0 space-y-6">
                <section v-show="activeSection === 'details'" class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <FormSelect label="Person" v-model="form.person_id" :error="form.errors.person_id" required>
                        ...
                        <FormSelect label="Position" v-model="form.position_id" :error="form.errors.position_id" required>
                        ...
                        <FormSelect label="Candidate Status" v-model="form.status" :error="form.errors.status" required>
                        ...
                        <FormInput label="Candidate FBR" v-model="form.candidate_fbr" type="number" step="0.01" :error="form.errors.candidate_fbr" />
                        <FormInput label="Submitted At" v-model="form.submitted_at" type="datetime-local" :error="form.errors.submitted_at" />
                        <FormSelect label="Submitted By" v-model="form.submitted_by_person_id" :error="form.errors.submitted_by_person_id">
                            <option value="">Select person</option>
                            <option v-for="person in people" :key="person.id" :value="person.id">{{ person.full_name ?? `${person.first_name} ${person.last_name}` }}</option>
                        </FormSelect>
                        <FormInput label="Scheduled Start Date" v-model="form.scheduled_start_date" type="date" :error="form.errors.scheduled_start_date" />
                    </div>
                </section>
                ...
                <section v-show="activeSection === 'workflow'" class="rounded-xl border bg-white p-6 shadow-sm">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <FormSelect v-if="can('view_admin')" label="Selected Workflow" v-model="selectedWorkflowId" @change="changeWorkflow">
                            <option v-for="item in workflows" :key="item.id" :value="item.id">{{ item.name }}{{ item.is_primary ? ' (Primary)' : '' }}</option>
                        </FormSelect>
                        <div v-else><Label>Selected Workflow</Label><div class="mt-2 rounded-md border bg-muted/40 px-3 py-2 text-sm">{{ workflow?.name || '—' }}</div></div>
                        <div><Label>Workflow Code</Label><div class="mt-2 rounded-md border bg-muted/40 px-3 py-2 text-sm">{{ workflow?.code || '—' }}</div></div>
                    </div>
...
const FormInput=defineComponent({props:{label:String,modelValue:[String,Number],error:String,type:{type:String,default:'text'},step:String,required:Boolean},emits:['update:modelValue'],setup(p,{emit}){return()=>h('div',[h('label',{class:'text-sm font-medium'},[p.label,p.required?h('span',{class:'ml-1 text-destructive'},'*'):null]),h('input',{class:'mt-2 flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm',type:p.type,step:p.step,value:p.modelValue,onInput:e=>emit('update:modelValue',e.target.value)}),p.error?h('p',{class:'mt-1 text-sm text-destructive'},p.error):null])}})
const FormSelect=defineComponent({props:{label:String,modelValue:[String,Number],error:String,required:Boolean},emits:['update:modelValue','change'],setup(p,{emit,slots}){return()=>h('div',[h('label',{class:'text-sm font-medium'},[p.label,p.required?h('span',{class:'ml-1 text-destructive'},'*'):null]),h('select',{class:'mt-2 flex h-10 w-full rounded-md border bg-background px-3 py-2 text-sm',value:p.modelValue,onChange:e=>{emit('update:modelValue',e.target.value);emit('change',e)}},slots.default?.()),p.error?h('p',{class:'mt-1 text-sm text-destructive'},p.error):null])}})
</script>
