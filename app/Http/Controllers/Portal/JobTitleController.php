...
class JobTitleController extends Controller
{
    ...
    public function index()
    {
        ...
        $preference = \App\Models\UserListPreference::query()
            ->where('user_id', request()->user()->id)
            ->where('list_key', 'portal_job_titles')
            ->first();

        return Inertia::render('Portal/JobTitles/Index', [
            'jobTitles' => $jobTitles,
            'columns' => $columns,
            'visibleColumns' => $preference?->visible_columns ?? collect($columns)->pluck('key')->all(),
            'columnOrder' => $preference?->column_order ?? collect($columns)->pluck('key')->all(),
        ]);
    }
    ...
    public function requirementsIndex()
    {
        ...
        return Inertia::render('Portal/JobTitleRequirements/Index', [
            'jobTitles' => $jobTitles,
        ]);
    }
    ...
    {
        $validKeys = ['name', 'description', 'skills_count', 'tasks_count', 'positions_count', 'is_active'];
        ...
        \App\Models\UserListPreference::updateOrCreate(
            [
                'user_id' => $request->user()->id,
                'list_key' => 'portal_job_titles',
            ],
            [
                'visible_columns' => collect($validated['visible_columns'])->filter(fn ($key) => in_array($key, $validKeys, true))->values()->all(),
                'column_order' => collect($validated['column_order'])->filter(fn ($key) => in_array($key, $validKeys, true))->values()->all(),
            ]
        );

        return redirect()->route('portal.job-titles.index')->with('success', 'Column preferences saved.');
    }

    public function resetPreferences(Request $request)
    {
        \App\Models\UserListPreference::query()
            ->where('user_id', $request->user()->id)
            ->where('list_key', 'portal_job_titles')
            ->delete();

        return redirect()->route('portal.job-titles.index')->with('success', 'Column preferences reset to defaults.');
    }

    public function create()
    {
        return Inertia::render('Portal/JobTitles/Create', [
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('portal.job-titles.show', $jobTitle)
            ->with(
                'success',
                $cloneSourceId
    ...
    public function show(JobTitle $jobTitle)
    {
        ...
        return Inertia::render('Portal/JobTitles/Show', [
            'jobTitle' => $jobTitle,
        ]);
    }
    ...
    {
        return Inertia::render('Portal/JobTitles/Edit', [
            'jobTitle' => $jobTitle,
        ]);
    }
