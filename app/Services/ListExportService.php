...
class ListExportService
{
        ...
        return Response::streamDownload(function () use ($rows, $activeColumns, $definition) {
            ...
            foreach ($rows as $rowModel) {
                ...
                foreach ($activeColumns as $column) {
                    $row[] = $this->resolveExportValue($rowModel, $column, $definition);
                }
    ...
    protected function resolveExportValue($rowModel, array $column, array $definition): string
    {
        ...
        if ($listKey === 'user_permissions') {
            return $this->resolveUserPermissionsExportValue($rowModel, $key);
        }
    ...
    protected function resolveUserPermissionsExportValue($user, string $key): string
    {
        return match ($key) {
            ...
            'roles' => $user->roles && $user->roles->count()
                ? $user->roles
                    ->map(fn ($role) => $role->label ?: $role->name)
                    ->implode('; ')
                : '',

            'permissions' => $user->permissions && $user->permissions->count()
                ? $user->permissions
